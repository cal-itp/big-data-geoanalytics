...
## Analysis Parameters
A full step-by-step breakdown of the analysis is in [_analysis_steps.md](./_analysis_steps.md)
<br></br>
For this analysis, we chose Spring 2025 and set the trip origin filter within a set geometry (either RTPA, Caltrans District, or County). We also filtered the data to trips longer than 45 minutes. We then change the data view as Trips by Origin and filtered the data to the Top 50 Trips by Origin within the previously set geomety. We used H3 Hex Cells Resolution 7, which is roughly 2square miles/cell as the geographical breakdown for a more granular analysis. Once the Origin filter was set up as those Top 50 Origins for the defined region, all trips with an origin as one of those tracts is exported. We then conducted the same analysis with Trips by Destination. Each trip dataset was downloaded from Replica and used in this analysis to easily compare the trip differences in trips across California.

<br>

To access the Statewide H3 Hex Cells Resolution 7 Layer used in the analysis, see [hex7_layer.zip](./hex7_layer.zip)
